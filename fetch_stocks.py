...
STOCK_INFO = {
    ...
    '2324':  ('仁寶',           '筆電組裝',   False),
    '2376':  ('技嘉',           'AI伺服器',   False),
    '2301':  ('光寶科',         '電源/光電',  False),
    '2354':  ('鴻準',           '散熱/機殼',  False),
    '6505':  ('台塑化',         '石化',       False),
    '3231':  ('緯創',           'AI伺服器',   False),
    '2395':  ('研華',           '工業電腦',   False),
    '1301':  ('台塑',           '石化',       False),
    '1303':  ('南亞',           '石化/玻纖布',False),
    '1326':  ('台化',           '石化',       False),
    '2002':  ('中鋼',           '鋼鐵',       False),
    '1101':  ('台泥',           '水泥',       False),
    '1216':  ('統一',           '食品',       False),
...
def get_monthly_data_tpex(stock_id, yyyymm):
    ...
    url = f"https://www.tpex.org.tw/web/stock/aftertrading/daily_trading_info/st43_result.php?l=zh-tw&d={date_str}&stkno={stock_id}&s=0,asc"
    try:
        res = requests.get(url, timeout=10, headers={'User-Agent': 'Mozilla/5.0'})
        data = res.json()
...
def get_monthly_data(stock_id, yyyymm, is_otc):
    if is_otc:
