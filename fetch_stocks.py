...
def get_today_prices_tpex():
    """從 TPEx 開放 API 抓今日所有上櫃股票收盤價"""
    try:
        ...
        prices = {}
        for item in r.json():
            code_ = item.get("SecuritiesCompanyCode", "")
            close_ = item.get("Close", "")
            if code_ and close_ and close_ not in ["-", "--", ""]:
                try:
                    prices[code_] = float(close_.replace(",", ""))
                except:
                    pass
        print(f"TPEx 今日收盤：取得 {len(prices)} 支")
        return prices
    except Exception as e:
        print(f"TPEx 今日收盤抓取失敗: {e}")
        return {}

# 預先抓取 TPEx 今日所有上櫃收盤價
today_prices_tpex = {}
try:
    _r = requests.get("https://www.tpex.org.tw/openapi/v1/tpex_mainboard_daily_close_quotes",
                      timeout=15, headers={'User-Agent': 'Mozilla/5.0'})
    if _r.status_code == 200:
        for _item in _r.json():
            _code = _item.get('SecuritiesCompanyCode', '')
            _close = _item.get('Close', '')
            if _code and _close and _close not in ['-', '--', '']:
                try:
                    today_prices_tpex[_code] = float(_close.replace(',', ''))
                except:
                    pass
        print(f"TPEx 今日收盤：取得 {len(today_prices_tpex)} 支上櫃股票")
except Exception as _e:
    print(f"TPEx 今日收盤抓取失敗: {_e}")
...
tpex_today = {}
if now.hour >= 14:
    tpex_today = get_today_prices_tpex()
...
for code in STOCK_LIST:
    ...
    try:
        ...
        # 上櫃股票用 TPEx 今日收盤價補正（yfinance 有一天延遲）
        today_str = now.strftime('%Y-%m-%d')
        if is_otc and code in tpex_today and unique_rows[-1]['date'] < today_str:
            today_close = tpex_today[code]
            last = unique_rows[-1]
            unique_rows.append({
                'date': today_str,
                'open': last['close'],
                'high': max(last['close'], today_close),
                'low':  min(last['close'], today_close),
                'close': today_close,
                'volume': 0,
            })
        ...
        # 上櫃股票用 TPEx 今日收盤價補正（yfinance 有一天延遲）
        today_str = now.strftime('%Y-%m-%d')
        if is_otc and code in today_prices_tpex:
            today_close = today_prices_tpex[code]
            if unique_rows and unique_rows[-1]['date'] < today_str:
                unique_rows.append({
                    'date': today_str, 'open': today_close,
                    'high': today_close, 'low': today_close,
                    'close': today_close, 'volume': 0,
                })

        last_date = unique_rows[-1]['date']
        print(f"  {code} 最後資料日期: {last_date} 收盤: {unique_rows[-1]['close']}")
        ...
        # 計算費波：同時算 60天 和 120天
        def calc_fib_levels(rows_slice):
            fh = max(r['high'] for r in rows_slice)
            fl = min(r['low']  for r in rows_slice)
            rng = fh - fl
            levels = []
            for ratio in [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1]:
                levels.append({'ratio': ratio, 'price': round(fh - rng * ratio, 2)})
            return levels

        fib_levels_60  = calc_fib_levels(unique_rows[-60:])
        fib_levels_120 = calc_fib_levels(unique_rows[-120:]) if len(unique_rows) >= 120 else fib_levels_60
        fib_levels = fib_levels_60  # 預設用60天

        def make_candles(rows_slice):
            return [{'date': r['date'], 'open': r['open'], 'high': r['high'],
                     'low': r['low'], 'close': r['close'], 'volume': r['volume']}
                    for r in rows_slice]

        candles    = make_candles(unique_rows[-120:]) if len(unique_rows) >= 120 else make_candles(unique_rows)
        ...
        # 近5日強度歷史（用來判斷趨勢是上升或下降）
        def calc_day_score(idx):
            if idx < 2: return None
            c_list = closes[:idx+1]
            v_list = vols[:idx+1]
            kd_s = kd_series[:idx+1]
            score = 0
            # hitFib
            c_price = c_list[-1]
            for f in fib_levels_60[1:-1]:
                if f['price'] > 0 and abs(c_price - f['price']) / f['price'] * 100 < 1.5:
                    score += 1; break
            # KD低檔
            k_i = kd_s[-1][0]
            if k_i < 30: score += 1
            # KD黃金交叉
            if len(kd_s) >= 3:
                k2,d2 = kd_s[-3]; k1,d1 = kd_s[-2]; ki,di = kd_s[-1]
                if (k2 <= d2) and (ki > di) and (ki < 60): score += 1
            # MA黃金交叉
            if len(c_list) >= 21:
                m5 = sum(c_list[-5:])/5; m20 = sum(c_list[-20:])/20
                m5p = sum(c_list[-6:-1])/5; m20p = sum(c_list[-21:-1])/20
                if m5p <= m20p and m5 > m20: score += 1
            # 量增
            if len(v_list) >= 20:
                avg20 = sum(v_list[-20:])/20
                if v_list[-1] > avg20 * 1.5: score += 1
            # K止跌回升
            if len(kd_s) >= 3:
                k2 = kd_s[-3][0]; k1 = kd_s[-2][0]; ki = kd_s[-1][0]
                if k2 < k1 < ki and ki < 40: score += 1
            return score
        ...
        for i in range(max(2, n-5), n):
            s = calc_day_score(i)
            if s is not None:
                score_history.append({'date': unique_rows[i]['date'], 'score': s})
        ...
        result[code] = {
            ...
            'fibLevels60': fib_levels_60,
            'fibLevels120': fib_levels_120,
            'candles': candles,
